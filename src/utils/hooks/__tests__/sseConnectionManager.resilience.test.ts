...
describe("sseConnectionManager — a refresh races a tenant change", () => {
  beforeEach(() => {
    (global as any).BroadcastChannel = MockBroadcastChannel;
    setLocks({
      request: jest.fn((_name: string, _opts: any, cb: () => Promise<void>) =>
        cb()
      ),
    });
  });

  it("a refresh that completes after a tenant change leaves the new tenant's token and watchdog alone", async () => {
    const streamB = scriptedStream();
    ...
    // already the active, connected scope.
    refreshResolve("A2");
    await flush();
    expect(authHeaders).not.toContain("Bearer A2");

    // Go silent on B's live stream; only a healthy watchdog forces the
    // reconnect that follows. B's original connect is signals[1] (index 0 is
    // tenant A's aborted-by-rescope connect attempt).
    await advance(30_000);
...
describe("sseConnectionManager — catch-up when a follower becomes leader", () => {
  let grantLock: () => void;
  ...
  beforeEach(() => {
    ...
    setLocks({
      request: jest.fn(
        (_name: string, _opts: any, cb: () => Promise<void>) =>
  ...
  it("a follower that takes over after the leader leaves asks the views to catch up", async () => {
    const stream = scriptedStream();
    const { fetchMock } = fetchReturning([stream]);
    global.fetch = fetchMock as any;
    ...
    const pollAlerts = jest.fn();
    const incidentChange = jest.fn();
    bindSSEHandler("connected", connected);
    bindSSEHandler("poll-alerts", pollAlerts);
    bindSSEHandler("incident-change", incidentChange);
    ...
    // This tab has heard from the outgoing leader on the channel before it
    // ever gets the lock itself.
    const channel = MockBroadcastChannel.instances[0];
    channel.emit({ kind: "keep-sse-leader-state", visible: true });
    ...
    expect(pollAlerts).toHaveBeenCalledWith({});
    expect(incidentChange).toHaveBeenCalledWith({});
    expect(pollAlerts.mock.invocationCallOrder[0]).toBeGreaterThan(
      connected.mock.invocationCallOrder[0]
    );
    expect(incidentChange.mock.invocationCallOrder[0]).toBeGreaterThan(
      connected.mock.invocationCallOrder[0]
    );
  });

  it("a tab that finds no leader does not catch up on its first stream", async () => {
    const stream = scriptedStream();
    const { fetchMock } = fetchReturning([stream]);
    global.fetch = fetchMock as any;
    const connected = jest.fn();
    ...
    // No broadcast or leader-state message arrived before we became leader.
    grantLock();
    await flush();
